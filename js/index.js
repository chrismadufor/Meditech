...
function changePage(page, rows = appointments) {
  ...
  for (
    ...
    i < page * records_per_page && i < rows.length;
    i++
  ) {
    if (userObj[0].userType == "Patient") {
      ...
      tableRow.innerHTML = `
            ...
            <td>
                <div class="tableTag ${rows[i].status.toLowerCase()}">${
        rows[i].status.charAt(0).toUpperCase() + rows[i].status.slice(1)
      }</div>
            </td>`;
    ...
    if (userObj[0].userType == "Doctor") {
      ...
      tableRow.innerHTML = `
            ...
            <td>
                <div class="tableTag ${rows[i].status.toLocaleLowerCase()}">${
        rows[i].status.charAt(0).toUpperCase() + rows[i].status.slice(1)
      }</div>
            </td>`;
      tableBody.append(tableRow);
